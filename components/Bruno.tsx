      ...
      {open && (
        <div className="mb-2 w-80 sm:w-96 h-[28rem] bg-white rounded-lg shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
          <div className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between">
            <div>
              <span className="font-semibold">BRUNO · Assistente</span>
              <p className="text-xs text-slate-400">
                {aiAttiva ? "Modalità AI attiva" : "Modalità ricerca (senza AI)"}
              </p>
            </div>
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto p-3 space-y-2">
            {messages.length === 0 && (
              <p className="text-sm text-slate-400 text-center mt-8">
                Ciao, sono BRUNO 👋
                <br />
                Scrivimi il nome di un cliente, un numero ordine, oppure chiedimi le statistiche
                generali.
              </p>
            )}
            {messages.map((m, i) => (
